...
This repository records `reviewPolicy: no-advisory`. F3 must not perform
the imported Copilot advisory recheck or require advisory convergence.
Retain the CI, branch-protection, review-currency, unresolved-thread,
unreplied-comment, and claim gates. The repository-local policy and this
override take precedence over Copilot-specific paragraphs below.
