...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `mid-llm-test-blocked-by` marker, both resolved the
...
NOT contain a `mid-llm-test-roadmap-id` marker (not itself
a roadmap) or a `mid-llm-test-blocked-by` marker, AND
...
`mid-llm-test-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`mid-llm-test-roadmap-id` marker. Retro-label a legacy
...
`<!-- mid-llm-test-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `mid-llm-test-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `mid-llm-test-roadmap-id` or
  `mid-llm-test-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `mid-llm-test-roadmap-id` marker when checking
  `mid-llm-test-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- mid-llm-test-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- mid-llm-test-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- mid-llm-test-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- mid-llm-test-effort: S|M|L -->` footer
...
- **Roadmap identity** (`mid-llm-test-roadmap-id`): placed in
...
- **Sequential dependency** (`mid-llm-test-blocked-by`): placed
...
**Do not use `mid-llm-test-blocked-by` to group sub-tasks under
