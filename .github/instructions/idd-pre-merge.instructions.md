...
## Local profile override — no-advisory

This repository records `reviewPolicy: no-advisory`. Ignore the imported
Copilot advisory-wait, Copilot convergence, and advisory-only disposition
requirements below. F2 must not request or wait for an advisory reviewer.
The effective F2 gates are review-currency freshness, current CI, branch
protection, required human-review states when configured, unresolved
conversation handling, unreplied non-agent comments, and active claim
ownership. If this override conflicts with imported Copilot-specific text,
this repository-local override wins.
