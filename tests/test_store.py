...
class StoreTests(unittest.TestCase):
  ...
  def test_unsupported_schema_and_duplicate_ids_are_rejected(self) -> None:
    for value in (99, True, 1.0):
      with self.subTest(schema_version=value):
        self.path.write_text(
          json.dumps({"schema_version": value, "tasks": []}),
          encoding="utf-8",
        )
        with self.assertRaisesRegex(
          LedgerError, "schema version is unsupported"
        ):
          Ledger(self.path).load()
  ...
  def test_unicode_write_failure_preserves_existing_file(self) -> None:
    ledger = Ledger(self.path)
    ledger.save([Task("task-1", "Original")])
    original = self.path.read_bytes()

    with self.assertRaisesRegex(LedgerError, "could not write ledger"):
      ledger.save([Task("task-1", "\ud800")])
