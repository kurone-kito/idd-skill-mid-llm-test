...
| 7 | #6 / A5-B3 | implementation and local validation passed | The claim `26916f84-f843-402a-a3bb-78d5c5c0c109` and activation nonce were verified before B2. The plan critique made the subprocess test use `sys.executable`, capture generated IDs, and isolate malformed-ledger reads on a separate path. The end-to-end test now covers add, deterministic list, due/tag/repeat queries, done, restore, persisted JSON, and clean malformed-input errors. All 26 tests, cspell, markdownlint, and `git diff --check` pass locally. Several hand-entered source-root/worktree paths again failed before process creation, so no repository mutation occurred. One initial post-claim resume check omitted `--claim-id`/`--nonce` and reported the expected non-inheritable route; the corrected ownership check reported `already_owned` with the matching nonce. This was an invocation error, not an IDD defect. The issue body still has stale prose saying it is blocked by completed children, while roadmap graph readiness is correct. | no new upstream issue; continue using #1860 and target #9 |
| 8 | #6 / E1-E13 | three review findings accepted; fix validated locally | The first connector-based Copilot request returned success but did not produce a visible review request; the CLI fallback produced a `review_requested` event tied to the current head, after which the advisory helper reported `copilotPendingCoversHead: true`. Copilot then reviewed `41e43b2`; a Codex review added three actionable test threads. Verification accepted all three: the test now preserves malformed-ledger bytes, adds a future `work`+`focus` daily control row for `--overdue`, and compares the complete ID list without set conversion. The first `python3 -m unittest discover -v` invocation ran zero tests because the repository requires `-s tests`; the corrected command ran all 26 tests. An advisory-state invocation without explicit `--owner`/`--repo` resolved the PR number in the source repository and failed; the explicit target form succeeded. The earlier E9 patch was also applied before its immediate claim revalidation gate, an instruction violation recorded here for the experiment rather than treated as an upstream defect. | no new upstream issue; #1860 remains the verified onboarding defect and #9 remains the target follow-up |
| 9 | #6 / E2-E7 | dispositions verified; merge-bound refresh pending | The independent critique found no new defect in b126bb7. Three Codex inline findings were accepted with individual `**Accepted**` replies and resolved; the verifier then reported zero missing thread or regular-comment disposition evidence after the Copilot and CodeRabbit items were handled separately. Copilot's current-head suppressed CRLF suggestion was verified false because `text=True` normalizes CRLF before `rstrip("\\n")`; a reasoned `**Rejected**` reply records the reproduction. CodeRabbit's current `review in progress` message was handled as a non-review notice, never as completed evidence. A first verifier invocation embedded backticks in a shell argument and triggered command substitution errors before the helper ran; no mutation occurred, and a safe rerun passed all five modeled items. The current watermark must be refreshed after these disposition comments before F2. | no new upstream issue; continue with #1860 and target #9 |
| 10 | #12 / B2-B3 | validated locally; PR pending | The post-merge Codex review of #11 exposed four concrete test-isolation gaps: due-on/tag intersection, completed-status exclusion, tag-mode all, and repeat daily. The refined plan added orthogonal controls and exact ID comparisons while keeping the fixture offline and standard-library only. The updated fixture creates eight controlled tasks with IDs captured at runtime; 26 tests, npx cspell, npx markdownlint-cli2, and `git diff --check` pass. Bare cspell and markdownlint binaries are absent in the dependency-free target, so the CI-equivalent npx commands were used. | no new upstream issue; the verified source onboarding gap remains #1860 |
