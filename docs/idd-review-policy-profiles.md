...
## Local selection: no-advisory

This experiment selects `no-advisory` because the public evaluation
repository has no IDD-managed advisory reviewer and no protected branch
or required check was visible during onboarding. The local phase files
carry an explicit override that disables Copilot request/wait/recheck
paths while retaining CI, review currency, unresolved-conversation, and
claim gates. Verification evidence and the selected merge policy are
recorded in [`docs/tier-a-experiment.md`](tier-a-experiment.md).
