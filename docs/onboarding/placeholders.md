...
| -------------------------------- | --------------------------------------------------------- | ---------------------------------- |
| `idd-skill-mid-llm-test`                  | Repository short name used in worktree examples           | `my-app`                           |
| `mid-llm-test`      | Hidden issue-body marker prefix                           | `my-app`                           |
| `kurone-kito`       | Single JSON-escaped login allowed to post trusted markers | `trusted-user-a`                   |
| `true`      | Auto-fix plus validate command row                        | `npm run lint:fix && npm run lint` |
| `true` | Non-mutating verify command row                           | `npm run lint && npm run test`     |
| `true` | Post-fix validate command row                             | `npm run lint:fix && npm test`     |
...
`mid-llm-test` appears in two hidden issue-body markers:

- roadmap identity marker:
  `<!-- mid-llm-test-roadmap-id: {unique-id} -->`
- blocked-by marker:
  `<!-- mid-llm-test-blocked-by: {roadmap-id} -->`
